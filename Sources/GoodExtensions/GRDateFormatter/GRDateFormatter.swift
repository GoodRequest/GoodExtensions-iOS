...
public extension GRDateFormatter {
    ...
    func isCurrentTimeBetween(startTime: String, endTime: String) -> Bool {
    ...
    /// Extracts the hours and minutes from a string formatted as "HH:mm".
    /// - Parameter key: The string from which to extract the hours and minutes.
    /// - Returns: A tuple containing the extracted hours and minutes, or `nil` if the input string is not in the expected format.
    func hoursminutesFromString(key: String) -> (hours: Int, minutes: Int)? {
        formatter.dateFormat = "HH:mm"
        guard let customDate = formatter.date(from: key) else { return nil }

        let calendar = Calendar.current

        let components = calendar.dateComponents([.hour, .minute], from: customDate)
